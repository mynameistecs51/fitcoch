...
class DiscussionService
{
    ...
    public function getModulePanel(int $moduleId, int $userId): array
    {
        ...
        if ($this->authzService->hasAnyRole($userId, ['instructor', 'admin'])) {
            $this->discussionRepo->markModuleRead($userId, $moduleId);
        }
    ...
    public function countUnreadByCourseIds(int $userId, array $courseIds): array
    {
        if (!$this->authzService->hasAnyRole($userId, ['instructor', 'admin'])) {
            return [];
        }

        return $this->discussionRepo->countUnreadLearnerPostsByCourseIds($userId, $courseIds);
    }
